...
class Indice extends ZoneCliquable {
    ...
    public void clicGauche() {
    	boolean isPossible = plateau.checkSolubility();
    	if (isPossible) {
    		this.setBackground(Color.GREEN);
    		plateau.plateau[plateau.rowIdx][plateau.colIdx].setBackground(Color.BLUE);
	    } else {
    		this.setBackground(Color.RED);
	    }
    }
...
class Plateau extends Grille {
    ...
    public boolean verifieConfiguration() {
    	return (checkColumns() && checkLines() && verifieDiagonales() && verifieAntidiagonales());
    }

    private int countLine(Case[] l) {
    	int nb = 0;
	    for (Case aCase : l) {
		    if (aCase.estOccupee()) {
			    nb++;
    ...
    private int countColumn(int c) {
    	int nb = 0;
    	for (int i = 0; i < plateau[0].length; i++) {
    		if (plateau[i][c].estOccupee()) {
    			nb++;
		    }
	    }
    	return nb;
    ...
    public boolean checkSolubility() {
    	if (!verifieConfiguration()) {
    		return false;
	    } else {
    		int r;
    		for (r = 0; r < taille && this.countLine(plateau[r]) != 0; r++) {}
    		if (r >= taille) {
    			return true;
		    } else {
    			boolean solFound = false;
    			for (int c = 0; c < taille && !solFound; colIdx = c++) {
    				this.plateau[r][c].isOccupied = true;
    				solFound = checkSolubility();
    				this.plateau[r][c].isOccupied = false;
			    }
    			rowIdx = r;
    			return solFound;
		    }
...
class Case extends ZoneCliquable {
    ...
    public boolean estOccupee() { return isOccupied; }
